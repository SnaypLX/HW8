...
def add_new_contact():
    ...
    if not exist_contact(0, " ".join(answers)):
        ...
        print("The entry has been succesfully added to the phone book!\n")
    else:
...
def data_collection(num):
    ...
    while True:
        if num in "surname name patromymic":
            if answer.isalpha():
                break
        if num == "phone number":
        ...
        answer = input(f"Data is incorrect!\n"
                       f"Use only use only letters"
                       f" of the alphabet, the length"
                       f" of the number if 11 digits\n"
                       f"Enter a {num}: ")
    return answer
...
def exp_imp_menu():
    ...
    while True:
        ...
        match move:
            case "1":
                ipn_bd(input("Enter the na,e of the file: "))
            case "2":
                exp_bd(input("Enter tne na,e of the file: "))
            case "3":
                return 0
            case _:
                print("The data is not recognized, repeat the input.")
